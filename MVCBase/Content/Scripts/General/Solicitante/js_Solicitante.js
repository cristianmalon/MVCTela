$(document).ready(function () {
    ...
    var dataSource = new kendo.data.DataSource({
        ...
        transport: {
            read: {
                type: "POST",
                url: "/Solicitante/ListarPaginado",
            ...
            parameterMap: function (options, operation) {
                return JSON.stringify({
                    eSolicitante: {
                        ...
                        ESTADO: 'A',
                        ARECCOD: '054000',
                        EplDCgo: ($('#frmIndexSolicitante :input[id="idBuscarCargo"]').val() == '') ? '%' : $('#frmIndexSolicitante :input[id="idBuscarCargo"]').val(),
                        EplDNom: ($('#frmIndexSolicitante :input[id="idBuscarNombre"]').val() == '') ? '%' : $('#frmIndexSolicitante :input[id="idBuscarNombre"]').val(),
                    }
                });
            }
        },
    ...
    $("#idBuscarCargo").keypress(function (e) {
        if (e.which === 13) {
            $(this).attr("disabled", "disabled");
            $('#gridData').data('kendoGrid').dataSource.read();
            $('#gridData').data('kendoGrid').refresh();
