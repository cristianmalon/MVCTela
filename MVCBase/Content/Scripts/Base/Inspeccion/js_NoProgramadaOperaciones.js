$(document).ready(function () {
    ...
    if (inspecion == "") {
        ...
        if (coordinaciontecnica == 49 || coordinaciontecnica == 51) {
    ...
    */
    ...
    $("#CT_MAE_TIPO_LUEGAR_DS_XTIPO_LUGAR_DS").find("option[value='opcion_1']").remove();
    ...
    //Ubigeo Nacional
    if (tipo_inspeccion == 61 || tipo_inspeccion == 95 || tipo_inspeccion == 96 || tipo_inspeccion == 97 || tipo_inspeccion == 102 ||
    ...
    $("#ID_INSPECTOR_OTRO").css({ "width": "100%" }).kendoComboBox({
        dataTextField: "XNOMBRECOMPLETO",
        dataValueField: "ID_INSPECTOR"
    });
    $("#modalDiscrepancia").modal('hide');
    ...
    $("#btnAddDiscrepancia").click(function () {
        ...
        if (validaInspeccion()) {
            modalAjaxRequestGet2("/Discrepancia/Discrepancia", "", "", 'cord_tec=' + "operaciones");
        }
    });
    ...
    $("#XID_INSPECTOR_OTRO").kendoMultiSelect({
        dataTextField: "XNOMBRECOMPLETO",
        dataValueField: "ID_INSPECTOR",
        filter: "contains"
    });
        ...
        change: function () {
            ...
            if (cmb.selectedIndex < 0) {
                ...
                if ($.trim($("#CT_GENM_ACTIVIDAD_DS_XID_GIRO").data("kendoComboBox").value()) != "" && $("#CT_GENM_ACTIVIDAD_DS_XID_GIRO").data("kendoComboBox").value() != null) {
                    $.ajax({
                        datatype: 'json',
                        url: '/Inspeccion/ComboPersonaJuridicaXGiro',
                        type: 'POST',
                        contentType: "application/json",
                        data: JSON.stringify({
                    ...
                    $.ajax({
                        datatype: 'json',
                        url: '/Inspeccion/CargarTipoInspeccion',
                        type: 'POST',
                        contentType: "application/json",
                        data: JSON.stringify({
                            XID_GIRO: $.trim($("#CT_GENM_ACTIVIDAD_DS_XID_GIRO").data("kendoComboBox").value())
        ...
        change: function () {
            ...
            if (cmb.selectedIndex < 0) {
                ...
                if ($.trim($("#CT_GENM_ACTIVIDAD_DS_XID_TIPO_INSPECCION").data("kendoComboBox").value()) != "" && $("#CT_GENM_ACTIVIDAD_DS_XID_TIPO_INSPECCION").data("kendoComboBox").value() != null) {
                    ...
                    //function valida inspección base
                    if ($("#CT_GENM_ACTIVIDAD_DS_XID_TIPO_INSPECCION").val() == 61) {
                        var url = "/PlanVigilanciaDetalleMes/ComboInspeccionBase?ID_TIPO_INSPECCION=" + String((this.value() == "" || this.value() == null) ? "0" : this.value());
                        $.getJSON(url, function (data) {
                            $("#FLG_RESULT_INSPECCION_BASE").val(data.flg_result);
                            if (data.flg_result) {
                                var options = [{
                                    ID_INSPECTOR: 0,
                                    XNOMBRECOMPLETO: "[SELECCIONE]"
                                }];
                                $.each(data.T_GENM_INSPECTORES, function (i, item) {
                                    options.push({
                                        ID_INSPECTOR: item.ID_INSPECTOR,
                                        XNOMBRECOMPLETO: item.XNOMBRECOMPLETO
                                    });
                                });
                                var selectOtro = $("#ID_INSPECTOR_OTRO").data("kendoComboBox");
                                selectOtro.setDataSource(options);
                                selectOtro.value([]);

                                $("#LABEL_INSPECTOR_OTRO").html("Otros Inspectores");
                                $("#DIV_INSPECTOR_OTRO").show();
                                $("#divInspectorOtros").show();
                            }
                            else {
                                $("#DIV_INSPECTOR_OTRO").hide();
                                $("#divInspectorOtros").hide();
                            }
                        ...
                        multiselect = $("#ID_INSPECTOR_OTRO").data("kendoComboBox");
                        multiselect.setDataSource(inspectores);
                        multiselect.value([]);

                        inspectores = [];
                        multiselect = $("#XID_INSPECTOR_OTRO").data("kendoMultiSelect");
                        multiselect.setDataSource(inspectores);
                        multiselect.value([]);

                        $("#FLG_RESULT_INSPECCION_BASE").val("FALSE");
                        $("#DIV_INSPECTOR_OTRO").hide();
                        $("#divInspectorOtros").hide();
                    }
                }
    ...
    $("#ID_INSPECTOR_OTRO").css({ "width": "100%" }).kendoComboBox({
        ...
        change: function (e) {
            ...
            if (cmb.selectedIndex < 0) {
                ...
                if ($.trim($("#ID_INSPECTOR_OTRO").data("kendoComboBox").value()) != "" && $("#ID_INSPECTOR_OTRO").data("kendoComboBox").value() != null) {

                    var url = "/PlanVigilanciaDetalleMes/ComboInspeccionBase?ID_TIPO_INSPECCION=" + $("#CT_GENM_ACTIVIDAD_DS_XID_TIPO_INSPECCION").val() + /*"&INSPECTOR=" + $("#ID_INSPECTORES").val() +*/ "&INSPECTORPRINCIPAL=" + String((this.value() == "" || this.value() == null) ? "0" : this.value());
                    $.getJSON(url, function (data) {
                        $("#FLG_RESULT_INSPECCION_BASE").val(data.flg_result);
                        if (data.flg_result) {
                            //console.log(data.T_GENM_INSPECTORES);
                            ...
                            var multiselect = $("#XID_INSPECTOR_OTRO").data("kendoMultiSelect");
                            multiselect.setDataSource(inspectores);
                            multiselect.value([]);

                            $("#LABEL_INSPECTOR_OTRO").html(data.labelCombo);
                        }
...
function saveInspeccionTrans(flgCloseModal) {
    if (validaInspeccion()) {
        ...
        $.ajax({
            datatype: 'json',
            contentType: "application/json",
            url: '/Inspeccion/SaveInspeccionNoProgramada',
            type: 'POST',
...
function validaInspeccion() {
    ...
    //if (tipo_inspeccion_l == 144 || tipo_inspeccion_l == 145 || tipo_inspeccion_l == 161 || tipo_inspeccion_l == 61 ) {
    ...
    if ($("#FLG_RESULT_INSPECCION_BASE").val() == 'true') {
        if ($("#ID_INSPECTOR_OTRO").data("kendoComboBox").value() == null || $("#ID_INSPECTOR_OTRO").data("kendoComboBox").value() == "0") {
            flg = false;
            $('[aria-owns="ID_INSPECTOR_OTRO_listbox"]').parents("span").addClass("valError");
            objData.push({ "ID_INSPECTOR_OTRO": [{ ErrorMessage: "Debe seleccionar un inspector responsable" }] })
        }
        if ($("#XID_INSPECTOR_OTRO").data("kendoMultiSelect").value() == null || $("#XID_INSPECTOR_OTRO").data("kendoMultiSelect").value() == "") {
            flg = false;
            $('[aria-owns="XID_INSPECTOR_OTRO_taglist XID_INSPECTOR_OTRO_listbox"]').parent("span").addClass("valError");
            objData.push({ "XID_INSPECTOR_OTRO": [{ ErrorMessage: "Debe seleccionar la opción otros inspectores" }] })
        }
    }
    ...
    //Ubigeo Nacional
    if (tipo_inspeccion == 61 || tipo_inspeccion == 161) {
        ...
        if ($("#CT_GENM_DISTRITO_XDISTRITO").val().trim() == "") {
            ...
            objData.push({ "CT_GENM_DISTRITO_XDISTRITO": [{ ErrorMessage: "Debe seleccionar una Distrito" }] })
        }
    ...
    else if (tipo_inspeccion == 141 ) {

        if (tipo_inspeccion == 141) {
            if ($("#XMATRICULA").val().trim() == "") {
                flg = false;
                $('[aria-owns="XMATRICULA_listbox"]').parents("span").addClass("valError");
                objData.push({ "XMATRICULA": [{ ErrorMessage: "Debe seleccionar una Matricula" }] })
...
function ComboboxTipoLugar() {
    ...
    var tipo_inspeccion_l = $("#CT_GENM_ACTIVIDAD_DS_XID_TIPO_INSPECCION").val();
    if (tipo_inspeccion_l == 144 || tipo_inspeccion_l == 145 || tipo_inspeccion_l == 161 || tipo_inspeccion_l == 61) {
        $("#DivAerodromoFin").hide();
        $("#DivHelipuertoFin").hide();
    }
...
function changeTipoInspeccion(valor) {
    ...
    if (tipo_inspeccion == 61 || tipo_inspeccion == 161) {
        ...
        $("#DivProvincia").show();
        $("#DivDistrito").show();
        $("#lblMatricula").removeClass("asterisco");
        $("#lblDepartamento").addClass("asterisco");
        $("#lblProvincia").addClass("asterisco");
        $("#lblDistrito").addClass("asterisco");
        LimpiarTipoinspeccion(valor);
    }
        //Ubigeo Internacional
    else if (tipo_inspeccion == 143) {
        $("#DivPais").show();
        $("#DivCiudad").show();
        $("#lblMatricula").removeClass("asterisco");
        $("#lblPais").addClass("asterisco");
        $("#lblCiudad").addClass("asterisco");
        LimpiarTipoinspeccion(valor);
    }
        //Matricula, Helipuerto o Aerodromo
    else if (tipo_inspeccion == 142) {
        ...
        $("#lblMatricula").removeClass("asterisco");
        $("#lblMatricula").addClass("asterisco");
        $("#lblTipoLugar").addClass("asterisco");
        $("#lblAerodromoInicio").addClass("asterisco");
        $("#lblAerodromoFin").addClass("asterisco");
        $("#lblHelipuertoInicio").addClass("asterisco");
        $("#lblHelipuertoFin").addClass("asterisco");
        LimpiarTipoinspeccion(valor);
        //var op = $("#XCB_ID_UBIGEO").val();
        //$("#CT_MAE_TIPO_LUEGAR_DS_XTIPO_LUGAR_DS").find("option[value='" + op + "']").remove();
        //Solo Lugar
    } else if (tipo_inspeccion == 41 || tipo_inspeccion == 144) {
        $("#DivTipoLugar").show();
        $("#lblTipoLugar").addClass("asterisco");
        $("#lblMatricula").removeClass("asterisco");
        $("#lblAerodromoInicio").addClass("asterisco");
        ...
        $("#lblDistrito").addClass("asterisco");
        LimpiarTipoinspeccion(valor);
    } else if (tipo_inspeccion == 141 || tipo_inspeccion == 145) {
        $("#DivMatricula").show();
        $("#DivTipoLugar").show();
        $("#lblMatricula").removeClass("asterisco");
        if (tipo_inspeccion == 141) {
...
function LimpiarTipoinspeccion(idtipoinspeccion) {
    // alert(idtipoinspeccion)
    $("#XMATRICULA").data("kendoComboBox").value('');
    $("#CT_MAE_TIPO_LUEGAR_DS_XTIPO_LUGAR_DS").data("kendoComboBox").value('');

    $("#CT_GENM_PAIS_XPAIS").data("kendoComboBox").value('');
    $("#CT_GENM_CIUDAD_XCIUDAD").data("kendoComboBox").value('');
