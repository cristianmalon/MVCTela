                ...
                if ($.trim($("#CT_GENM_ACTIVIDAD_DS_ID_PERSONA_JURIDICA").data("kendoComboBox").value()) != "" && $("#CT_GENM_ACTIVIDAD_DS_ID_PERSONA_JURIDICA").data("kendoComboBox").value() != null) {
                    $.ajax({
                        ...
                        data: JSON.stringify({
                            ...
                            XAERODROMO: $.trim($("#CT_GENM_ACTIVIDAD_DS_ID_PERSONA_JURIDICA").data("kendoComboBox").value().split("^")[1]),
                            XHELIPUERTO: $.trim($("#CT_GENM_ACTIVIDAD_DS_ID_PERSONA_JURIDICA").data("kendoComboBox").value().split("^")[2])
                        }),
                        beforeSend: function () {
                            bloquoteObject();
                        },
                        success: function (data) {
                            ...
                            if (!data.rpta) {
                                ...
                                $("#CT_GENM_ACTIVIDAD_DS_XID_AERODROMO").val(data.XAERODROMO);
                                $("#CT_GENM_ACTIVIDAD_DS_ID_HELIPUERTO").val(data.IdHelipuerto);
                                $("#CT_GENM_ACTIVIDAD_DS_XID_HELIPUERTO").val(data.XHELIPUERTO);
...
function LimpiarTipoinspeccion(idtipoinspeccion) {
    ...
    if (!$("#CT_GENM_ACTIVIDAD_DS_ID_HELIPUERTO").val() > 0) {
        $("#CT_GENM_HELIPUERTO_XHELIPUERTO").data("kendoComboBox").value('');
    }
    if (!($("#CT_GENM_ACTIVIDAD_DS_ID_AERODROMO").val() > 0 || $("#CT_GENM_ACTIVIDAD_DS_ID_HELIPUERTO").val() > 0)) {
        $("#CT_MAE_TIPO_LUEGAR_DS_XTIPO_LUGAR_DS").data("kendoComboBox").value('');
    }
