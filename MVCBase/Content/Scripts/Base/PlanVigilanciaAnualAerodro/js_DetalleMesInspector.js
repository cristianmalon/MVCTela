$(document).ready(function () {
    ...
    $("#btnReProgramarInspeccion").click(function () {
        ...
        if (itemData != null) {
            if (!itemData.FLG_PLANIFICADO) {
                if (!itemData.FLG_REPROGRAMADO) {
                    bootbox.confirm("¿Segúro de reprogramar la inspección seleccionada?", function (result) {
                        if (result) {
                            ...
                            modalAjaxRequestGet(url, "", "", "Index=" + itemData.XID_PLAN_VIGI_ANUAL_DS + "&Giro=" + $.trim($("#XID_GIRO").data("kendoComboBox").value()));
                        }
                    });
                }
                else {
                    bootbox.alert("Este registro ya ha sido reprogramado en el plan anual!");
                }
            }
            else {
                bootbox.alert("Este registro ya ha sido planificado en el plan mensual!");
            }
        }
        else {
    ...
    $("#btnRegistrarNuevaInspeccionInMes").click(function () {
        modalAjaxRequestGet($(this).attr("data-url"), "", "", "Giro=" + $.trim($("#XID_GIRO").data("kendoComboBox").value()));
    });
...
function gridFiltroListaInspeccionMes() {
    ...
    var dataSource = new kendo.data.DataSource({
        transport: {
            read: {
                type: "POST",
                url: "/PlanVigilanciaAnualAerodro/ListaInspecionesMes",
            ...
            parameterMap: function (options, operation) {
                ...
                var o_T_Genm_Plan_Vigi_Anual_DS = {
                    ...
                    XID_AERODROMO: $.trim($("#XID_AERODROMO").val()),
                    XID_HELIPUERTO: $.trim($("#XID_HELIPUERTO").val())
                };
