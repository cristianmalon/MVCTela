$(document).ready(function () {
    $(window).load(function () {
            ...
            change: function () {
                ...
                if (cmb.selectedIndex < 0) {
                    ...
                    if ($.trim($("#XID_GIRO").data("kendoComboBox").value()) != "" && $("#XID_GIRO").data("kendoComboBox").value() != null) {
                        $.ajax({
                            datatype: 'json',
                            url: '/PlanVigilanciaAnualAerodro/GiroPersonaJuridicaInspector',
                            type: 'POST',
                            contentType: "application/json",
                            data: JSON.stringify({
        ...
        $("#btnRegistrarNuevaInspeccionIn").click(function () {
        ...
        $("#FrmPlanVigilanciaAnual").on("click", ".InspeccionesProgramadasMes", function () {
            ...
            $.ajax({
                url: '/PlanVigilanciaAnualAerodro/DetalleMesInspector?Index=' + Index,
                type: "GET",
                beforeSend: function () {
                    bloquoteObject();
                },
            ...
            if (contBlur == 1) {
                try {
                    if ($(this).val() >= 0) {
                        ...
                        $.ajax({
                            datatype: 'json',
                            url: '/PlanVigilanciaAnualAerodro/RegistroInspeccionInspectorCantidad',
                            type: 'POST',
                            contentType: "application/json",
                            data: JSON.stringify({
        ...
        $("#btnNotificarAprobacion").click(function () {
            $.ajax({
                datatype: 'json',
                url: '/PlanVigilanciaAnualAir/NotificacionCoordinador',
                type: 'POST',
                contentType: "application/json",
                data: JSON.stringify({
                    Giro: $.trim($("#XID_GIRO").data("kendoComboBox").value()),
            ...
            if ($("#XID_GIRO").data("kendoComboBox").value() == null || $("#XID_GIRO").data("kendoComboBox").value() == "") {
                ...
                if ($("#XID_PERSONA_JURIDICA").data("kendoComboBox").value() == null || $("#XID_PERSONA_JURIDICA").data("kendoComboBox").value() == "") {
                    ...
                    if ($("#ckAerodromo").is(':checked') == false && ($("#XAERODROMO").data("kendoComboBox").value() == null || $("#XAERODROMO").data("kendoComboBox").value() == "")) {
                        bootbox.alert("Debe seleccionar el aerodromo");
                    }
                    else {
                        loadGrilla();
                    }
...
function loadGrilla() {
    $.ajax({
        datatype: 'json',
        url: '/PlanVigilanciaAnualAerodro/FiltroPlanAnualInsp',
        type: 'POST',
        contentType: "application/json",
        data: JSON.stringify({
            Giro: $.trim($("#XID_GIRO").data("kendoComboBox").value()),
...
function onSelectGiro(e) {
    ...
    if ($("#XID_GIRO option:selected").val().trim().length != 0 && $("#XID_GIRO option:selected").val() != null) {
        if (dataItem.value != "" && dataItem.value != null) {
            $.ajax({
                datatype: 'json',
                url: '/PlanVigilanciaAnualAir/GiroPersonaJuridicaInspector',
                type: 'POST',
                contentType: "application/json",
                data: JSON.stringify({
