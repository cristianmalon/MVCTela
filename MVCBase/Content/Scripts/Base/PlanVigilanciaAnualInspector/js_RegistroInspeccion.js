...
$(document).ready(function () {
    ...
    $("#XID_TIPO_LUGAR_DS").kendoComboBox({
        placeholder: "[SELECCIONE]",
        dataTextField: "text",
        dataValueField: "value",
        ...
        change: function () {
            ...
            if (cmb.selectedIndex < 0) {
                ...
                if ($.trim($("#XID_TIPO_LUGAR_DS").data("kendoComboBox").value()) != "" && $("#XID_TIPO_LUGAR_DS").data("kendoComboBox").value() != null) {
                    changeTipoLugar($.trim($("#XID_TIPO_LUGAR_DS").data("kendoComboBox").value()));
                }
            }
        },
    });
            ...
            if (cmb.selectedIndex < 0) {
                ...
                if ($.trim($("#XDEPARTAMENTO").data("kendoComboBox").value()) != "" && $("#XDEPARTAMENTO").data("kendoComboBox").value() != null) {
                    if (contDepartamento == 0) {
                        contDepartamento++;
                        loadProvincia($.trim($("#XDEPARTAMENTO").data("kendoComboBox").value()));
                    }
                }
            }
        },
        ...
        change: function () {
            ...
            if (cmb.selectedIndex < 0) {
                ...
                if ($.trim($("#XPROVINCIA").data("kendoComboBox").value()) != "" && $("#XPROVINCIA").data("kendoComboBox").value() != null) {
                    loadDistrito($.trim($("#XPROVINCIA").data("kendoComboBox").value()));
                }
            }
        },
    });

    $("#XID_DISTRITO").kendoComboBox({
        placeholder: "[SELECCIONE]",
        dataTextField: "text",
        dataValueField: "value",
        filter: "contains",
    ...
    $("#btnGuardarInspeccionProgramada").click(function () {
        if (validRegistroInspección()) {
            var o_T_Genm_Plan_Vigi_Anual_DS = {
                ...
                FLG_NACIONAL: true,
                XID_TIPO_LUGAR_DS: $.trim($("#XID_TIPO_LUGAR_DS").data("kendoComboBox").value()),
                XID_ESTADO_PLAN_VIGI_ANUALDS: $.trim($("#XID_ESTADO_PLAN_VIGI_ANUALDS").data("kendoComboBox").value()),
                ...
                XID_LUGAR: GetValorTipoLugar($.trim($("#XID_TIPO_LUGAR_DS").data("kendoComboBox").value())),
                XID_DISTRITO: $("#XID_DISTRITO").data("kendoComboBox").value(),
            ...
            console.log(o_T_Genm_Plan_Vigi_Anual_DS);
            $.ajax({
                datatype: 'json',
                contentType: "application/json",
                url: '/PlanVigilanciaAnual/RegistroInspeccion',
                type: 'POST',
...
function validRegistroInspección() {
    ...
    if ($("#XID_TIPO_LUGAR_DS").data("kendoComboBox").value() == null || $("#XID_TIPO_LUGAR_DS").data("kendoComboBox").value() == "") {
        flg = false;
        $('[aria-owns="XID_TIPO_LUGAR_DS_listbox"]').parents("span").addClass("valError");
        objData.push({ "XID_TIPO_LUGAR_DS": [{ ErrorMessage: "Debe seleccionar el tipo lugar" }] })
    }
    else {
        var valor = $.trim($("#XID_TIPO_LUGAR_DS").data("kendoComboBox").value());
        if ($("#ValAerodromo").val() == valor) {
            if ($("#XAERODROMO").data("kendoComboBox").value() == null || $("#XAERODROMO").data("kendoComboBox").value() == "") {
                flg = false;
                $('[aria-owns="XAERODROMO_listbox"]').parents("span").addClass("valError");
                objData.push({ "XAERODROMO": [{ ErrorMessage: "Debe seleccionar el aerodromo" }] })
            }
        }
        if ($("#ValHelipuerto").val() == valor) {
            if ($("#XHELIPUERTO").data("kendoComboBox").value() == null || $("#XHELIPUERTO").data("kendoComboBox").value() == "") {
                flg = false;
                $('[aria-owns="XHELIPUERTO_listbox"]').parents("span").addClass("valError");
                objData.push({ "XHELIPUERTO": [{ ErrorMessage: "Debe seleccionar el helipuerto" }] })
            }
        }
        if ($("#ValUbigeo").val() == valor) {
            ...
            if ($("#XID_DISTRITO").data("kendoComboBox").value() == null || $("#XID_DISTRITO").data("kendoComboBox").value() == "") {
                flg = false;
                $('[aria-owns="XID_DISTRITO_listbox"]').parents("span").addClass("valError");
                objData.push({ "XID_DISTRITO": [{ ErrorMessage: "Debe seleccionar el distrito" }] })
            }
        }
    }
    if ($("#XID_ESTADO_PLAN_VIGI_ANUALDS").data("kendoComboBox").value() == null || $("#XID_ESTADO_PLAN_VIGI_ANUALDS").data("kendoComboBox").value() == "") {
        flg = false;
        $('[aria-owns="XID_ESTADO_PLAN_VIGI_ANUALDS_listbox"]').parents("span").addClass("valError");
        objData.push({ "XID_ESTADO_PLAN_VIGI_ANUALDS": [{ ErrorMessage: "Debe seleccionar el estado del plan de vigilancia anual" }] })
    }   
...
function loadProvincia(valor) {
    $.ajax({
        datatype: 'json',
        url: '/PlanVigilanciaAnual/CargarProvincia?Index=' + valor,
        contentType: "application/json",
        beforeSend: function () {
            bloquoteObject();
        },
...
function loadDistrito(valor) {
    $.ajax({
        datatype: 'json',
        url: '/PlanVigilanciaAnual/CargarDistrito?Index=' + valor,
        contentType: "application/json",
        beforeSend: function () {
            bloquoteObject();
        },
