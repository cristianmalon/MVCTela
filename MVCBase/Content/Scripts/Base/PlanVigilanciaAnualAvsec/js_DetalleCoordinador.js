        ...
        $("#XCIUDAD").kendoComboBox({
            ...
            change: function () {
                ...
                if (cmb.selectedIndex < 0) {
                    ...
                    if ($.trim($("#XCIUDAD").data("kendoComboBox").value()) != "" && $("#XCIUDAD").data("kendoComboBox").value() != null) {
                        $.ajax({
                            datatype: 'json',
                            ...
                            data: JSON.stringify({
                                XCIUDAD: $.trim($("#XCIUDAD").data("kendoComboBox").value())
                            }),
                            beforeSend: function () {
                                bloquoteObject();
                            },
            ...
            change: function () {
                ...
                if (cmb.selectedIndex < 0) {
                    ...
                    if ($.trim($("#XID_PERSONA_JURIDICA").data("kendoComboBox").value()) != "" && $("#XID_PERSONA_JURIDICA").data("kendoComboBox").value() != null) {
                        $.ajax({
                            datatype: 'json',
                            url: '/PlanVigilanciaAnualAvsec/DetalleCertificacion',
                            type: 'POST',
                            contentType: "application/json",
                            data: JSON.stringify({
        ...
        */
            ...
            change: function () {
                ...
                if (cmb.selectedIndex < 0) {
                    ...
                    if ($.trim($("#XID_GIRO").data("kendoComboBox").value()) != "" && $("#XID_GIRO").data("kendoComboBox").value() != null) {
                        $.ajax({
                            datatype: 'json',
                            url: '/PlanVigilanciaAnualAvsec/GiroPersonaJuridica',
                            type: 'POST',
                            contentType: "application/json",
                            data: JSON.stringify({
        ...
        $("#FrmPlanVigilanciaAnual").on("click", ".InspeccionesProgramadasMes", function () {
            ...
            var Index =   $(this).attr('data-month') + '^'
                        ...
                        + $(this).attr('data-giro') + '^'
                        + $(this).attr('data-habilitacion') + '^'
                        + $(this).attr('data-inspeccion') + '^'
                        + $(this).attr('data-ciudad') + '^'
                        + $("#XID_ESTADO_FILTRO_DS").data("kendoComboBox").value();

            $.ajax({
                url: '/PlanVigilanciaAnualAvsec/DetalleMesCoordinador?Index=' + Index,
                type: "GET",
                beforeSend: function () {
                    bloquoteModal();
                },
                            ...
                            success: {
                                ...
                                callback: function () {
                                    if ($.trim($("#txtObservacion").val()).length > 0) {
                                        ...
                                        $.ajax({
                                            datatype: 'json',
                                            url: '/PlanVigilanciaAnualAvsec/DesaprobarMasivoCoordinador',
                                            type: 'POST',
                                            contentType: "application/json",
                                            data: JSON.stringify({
        ...
        $("#FrmPlanVigilanciaAnual").on("blur", ".inputInspecciones", function () {
            ...
            var CiudadObject = $(this).attr('data-ciudad');
            var EmpresaObject = $(this).attr('data-empresa');
            var GiroObject = $(this).attr('data-giro');
            var HabilitacionObject = $(this).attr('data-habilitacion');
            var MonthObject = $(this).attr('data-month');
            var InspObject = $(this).attr('data-inspeccion');
            var CoodinacionTecGiroTipinspecObject = $(this).attr('data-coordinaciontecgiro-tipinspec');
            var CoordinacionTecGiroObject = $(this).attr('data-coordinaciontec-giro');
            var valObject = $(this).val();
                ...
                try {
                    if ($(this).val() >= 0) {
                        ...
                        var o_T_Genm_Plan_Vigi_Anual_DS = {
                            ...
                            FLG_PLANIFICADO: false,
                            FLG_CUMPLIDO: false,
                            XID_COORDITECGIRO_TIPINSPEC: CoodinacionTecGiroTipinspecObject,
                            XID_TIPO_INSPECCION: InspObject,
                            XID_COORDINACIONTEC_GIRO: CoordinacionTecGiroObject,
                            XID_GIRO: GiroObject,
                            XID_LUGAR: "",
                            XID_DISTRITO: "",
                            XID_TIPO_ACTIVIDAD_DS: "0",
                            XID_HABILITACION: HabilitacionObject,
                            XID_PERSONA_JURIDICA: EmpresaObject,
                            XID_COORDINACION_TECNICA: "0",
                            FLG_ESTADO: true,
                            XCIUDAD: CiudadObject
                        }

                        $.ajax({
                            datatype: 'json',
                            url: '/PlanVigilanciaAnualAvsec/RegistroInspeccionCoordinadorCantidad',
                            type: 'POST',
                            contentType: "application/json",
                            data: JSON.stringify({
                                Cantidad: valObject, o_T_Genm_Plan_Vigi_Anual_DS: o_T_Genm_Plan_Vigi_Anual_DS
        ...
        $("#btnFiltroPlanVigilanciaAnual").click(function () {
            if ($("#XCIUDAD").data("kendoComboBox").value() == null || $("#XCIUDAD").data("kendoComboBox").value() == "") {
                bootbox.alert("Debe seleccionar el lugar");
            }
            else {
                if ($("#XPERSONAJURIDICA").data("kendoMultiSelect").value() == null || $("#XPERSONAJURIDICA").data("kendoMultiSelect").value() == "") {
                    bootbox.alert("Debe seleccionar una empresa con su giro");
                }
                else {
                    var htmlConst = "";
                    ...
                    $.ajax({
                        datatype: 'json',
                        url: '/PlanVigilanciaAnualAvsec/FiltroPlanAnualCoord',
                        type: 'POST',
                        contentType: "application/json",
                        data: JSON.stringify({
                            Ciudad: $.trim($("#XCIUDAD").data("kendoComboBox").value()),
                            PersonaJuridica: $.trim($("#XPERSONAJURIDICA").data("kendoMultiSelect").value()),
                            Estado: $.trim($("#XID_ESTADO_FILTRO_DS").data("kendoComboBox").value()),
