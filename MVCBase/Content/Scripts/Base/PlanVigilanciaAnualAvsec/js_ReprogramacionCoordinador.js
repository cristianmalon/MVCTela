    ...
    $("#btnGuardarInspeccionProgramada").click(function () {
        if (validRegistroInspección()) {
            var o_T_Genm_Plan_Vigi_Anual_DS = {
                ...
                XID_ESTADO_PLAN_VIGI_ANUALDS: "0",
                ...
                FLG_ESTADO: true,
                OBSERVACION_REPROGRAMADO: $.trim($("#OBSERVACION_REPROGRAMADO").val())
            }
            
            $.ajax({
                datatype: 'json',
                contentType: "application/json",
                url: '/PlanVigilanciaAnualAvsec/ReprogramacionCoordinador',
                type: 'POST',
                ...
                success: function (data) {
                    if (!data.rpta) {
                        errorAddModelo("divErrorRegistro", "ulListaErrorRegistro", data.errores);
                    } else {
...
function validRegistroInspección() {
    ...
    if ($.trim($("#OBSERVACION_REPROGRAMADO").val()) == "") {
        flg = false;
        objData.push({ "OBSERVACION_REPROGRAMADO": [{ ErrorMessage: "Debe ingresar una observación" }] })
    }

    if (flg) {
        $("#divErrorRegistro").hide();
    }
    else {
        errorAddJS("divErrorRegistro", "ulListaErrorRegistro", objData)
    }
    return flg;
}
...
function loadProvincia(valor) {
    $.ajax({
        datatype: 'json',
        url: '/PlanVigilanciaAnualAir/CargarProvincia?Index=' + valor,
        contentType: "application/json",
        beforeSend: function () {
            bloquoteObject();
        },
...
function loadDistrito(valor) {
    $.ajax({
        datatype: 'json',
        url: '/PlanVigilanciaAnualAir/CargarDistrito?Index=' + valor,
        contentType: "application/json",
        beforeSend: function () {
            bloquoteObject();
        },
