$(document).ready(function () {
    $(window).load(function () {
        ...
        $("#FrmPlanVigilanciaAnual").on("click", ".InspeccionesProgramadasMes", function () {
            ...
            $.ajax({
                url: '/PlanVigilanciaAnualAvsec/DetalleMesInspector?Index=' + Index,
                type: "GET",
                beforeSend: function () {
                    bloquoteObject();
                },
            ...
            if (contBlur == 1) {
                try {
                    if ($(this).val() >= 0) {
                        ...
                        $.ajax({
                            datatype: 'json',
                            url: '/PlanVigilanciaAnualAvsec/RegistroInspeccionInspectorCantidad',
                            type: 'POST',
                            contentType: "application/json",
                            data: JSON.stringify({
        ...
        $("#btnNotificarAprobacion").click(function () {
            $.ajax({
                datatype: 'json',
                url: '/PlanVigilanciaAnualAvsec/NotificacionCoordinador',
                type: 'POST',
                contentType: "application/json",
                data: JSON.stringify({
        ...
        $("#btnFiltroPlanVigilanciaAnual").click(function () {
            if ($("#XID_GIRO").data("kendoComboBox").value() == null || $("#XID_GIRO").data("kendoComboBox").value() == "") {
                ...
                if ($("#XID_PERSONA_JURIDICA").data("kendoComboBox").value() == null || $("#XID_PERSONA_JURIDICA").data("kendoComboBox").value() == "") {
                    ...
                    $.ajax({
                        datatype: 'json',
                        url: '/PlanVigilanciaAnualAvsec/FiltroPlanAnualInsp',
                        type: 'POST',
                        contentType: "application/json",
                        data: JSON.stringify({
                            Giro: $.trim($("#XID_GIRO").data("kendoComboBox").value()),
