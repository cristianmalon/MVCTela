...
function gridAerodromoEmpresas(data, lObjeto) {
    ...
    var dataSource = new kendo.data.DataSource({
        ...
        schema: {
            model: {
                id: "XID_AERO_PER_JURID",
                fields: {
                    XID_AERO_PER_JURID: { editable: false, nullable: true },
    ...
    $("#gridAerodromoEmpresas").kendoGrid({
        ...
        height: 300,
        columns: [
            { field: "XID_AERO_PER_JURID", hidden: true },
            {
            ...
            {
                template: "#if (XID_AERO_PER_JURID  == '') {# <a class='k-button k-button-icontext k-grid-cancel' ><span class='k-icon k-cancel'></span>Cancelar</a> #} # ",                
                width: 65
            }
        ],
        editable: true
...
function ajaxAerodromo() {
    $.ajax({
        datatype: 'json',
        url: '/AeroPerJurid/ListarAerodromos',
        type: 'POST',
        ...
        if (gridAerodromoEmpresas != null) {
            $.each(gridAerodromoEmpresas, function (index, item) {
                ...
                var T_GEND_AERO_PER_JURID = {
                    ...
                    ID_USUARIO_ACT: null,
                    FEC_ACT: null,
                    XID_AERO_PER_JURID: item.XID_AERO_PER_JURID,
                    XID_PERSONA_JURIDICA: $("#txtIdPersonaJuridica").val().trim(),
                    OACI: item.ID_AERODROMO.OACI
                }

                ListAerodromoEmpresas.push(T_GEND_AERO_PER_JURID);
            });
        }
        console.log(ListAerodromoEmpresas);
    
    
        $.ajax({
            datatype: 'json',
            contentType: "application/json",
            url: '/AeroPerJurid/SaveRegistroAeroPerJuridXML',
            type: 'POST',
...
function EditGrillaAerodromoEmpresas(lObjeto) {
    ...
    $.ajax({
        datatype: 'json',
        url: '/AeroPerJurid/dataGrillaAerodromoEmpresas',
        type: 'POST',
        contentType: "application/json",
        data: JSON.stringify({
        ...
        success: function (data) {
            if (!data.rpta) {
                ...
                $.each(data.l_GEND_AERO_PER_JURID, function (index, item) {
                    $("#gridAerodromoEmpresas")
                            .data("kendoGrid")
                            .dataSource
                            .insert({
