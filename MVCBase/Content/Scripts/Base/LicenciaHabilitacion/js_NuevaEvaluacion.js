...
function CargarDatosKendoUi() {
    ...
    $("#XID_TIPO_EVALUACION_LIC").css({ "width": "100%", "height": "auto" }).kendoComboBox({
...
function validDatosEvaluacion() {
    ...
    if ($("#XID_TIPO_EVALUACION_LIC").data("kendoComboBox").value() == null || $("#XID_TIPO_EVALUACION_LIC").data("kendoComboBox").value() == "") {
        flg = false;
        $('[aria-owns="XID_TIPO_EVALUACION_LIC_listbox"]').parents("span").addClass("valError");
        objData.push({ "XID_TIPO_EVALUACION_LIC": [{ ErrorMessage: "Debe seleccionar el tipo de Evaluación" }] })
    }
