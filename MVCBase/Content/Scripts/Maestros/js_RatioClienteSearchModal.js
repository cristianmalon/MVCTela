$(document).ready(() => {
        ...
        transport: {
            ...
            parameterMap: function (options) {
                return JSON.stringify({
                    ...
                    IdTipoEmpaque: $("#frmListRatioCliente").find("input[id='idtipoempaque']").val()
                });
            }
        },
        schema: {
    ...
    $("#btnRegistro").on('click', function (e) {
        var SerialKey = '';
        var CodCliente= $("#frmListRatioCliente").find("input[id='codcliente']").val();
        var IdTipoEmpaque = $("#frmListRatioCliente").find("input[id='idtipoempaque']").val();
        BootstrapDialog.show({
            title: 'Mantenimiento de Ratios',
            cssClass: 'size-wide-dialog-1000',
            type: BootstrapDialog.TYPE_SUCCESS,
            message: $('<div><img src="../Content/images/loading_modal.gif" class="image-center" /></div>').load(Enrutamiento($(this).attr('data-url'), 'key=' + SerialKey + '&codcliente=' + CodCliente + '&idempaque='+ IdTipoEmpaque)),
            buttons: [
                {
                    label: 'Cerrar',
